...
#include <cgi4esl/client/consumer/ResponseHandler.h>
#include <cgi4esl/client/producer/RequestStatic.h>
#include <cgi4esl/client/producer/RequestDynamic.h>
#include <cgi4esl/client/producer/RequestFile.h>
#include <cgi4esl/Logger.h>
...
namespace {
Logger logger("cgi4esl::client::Connection");
}
...
std::unique_ptr<esl::http::client::Interface::Connection> Connection::create(const esl::utility::URL& hostUrl, const esl::object::Values<std::string>& values) {
	...
	if(values.hasValue("executable") == false) {
        throw esl::addStacktrace(std::runtime_error("cgi4esl: No executable specified."));
	}

	esl::system::process::Arguments arguments(values.getValue("executable"));
	if(arguments.getArgc() < 1) {
        throw esl::addStacktrace(std::runtime_error("cgi4esl: Specified executable is empty."));
	}
